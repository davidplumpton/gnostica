...
`src/main/gnostica/game_state.cljc` defines the browser-free gameplay state contract. Use `gnostica.game-state/create-game` when tests, Gherkin-style feature steps, or re-frame events need an authoritative state value with deterministic deck setup and explicit success/error results. It enforces two to six known players backed by `gnostica.pieces/players` metadata, deals six-card starting hands before the territory board, reuses `gnostica.board` for territory setup, validates injected deck cards for unique ids plus rendering fields, and stores phase, players, turn order, pieces/stashes, draw/discard piles, setup/bid metadata, and history events.

`gnostica.game-state/apply-cup-move` is the current pure gameplay move transition. Keep its command contract explicit: `:player-id`, a `:source` map for either `:territory` or `:hand-card` plus acting `:piece-id`, a `:target` map for either `:territory` or `:wasteland`, and the required `:orientation` or `:one-point-card-id`. It should continue to return structured success/error maps, update player and stash mirrors together, move hand-source cards to discard, create one-point-card wasteland territories without duplicating cards, append history events, and keep renderer/UI concerns out of `game_state.cljc`.
...
Current-player card-zone subscriptions also derive from `:game`: `gnostica.app-state/card-zones` exposes the current hand, draw pile, discard pile, counts, and top discard card for the browser UI. Keep hand/deck/discard rendering display-only and do not duplicate those zones at the app-db top level. The standalone draw/discard turn option still needs a future pure gameplay transition, but Cup hand-card moves already move the source card from hand to discard through `apply-cup-move`.

Move-selection UI state also belongs in `src/main/gnostica/app_state.cljc` under the top-level `:move-selection` key. Keep it as a staged, browser-free state machine over the authoritative `:game` data: source choice, source territory, hand card, minion/piece, target territory or wasteland, one-point card, orientation, draw count, confirmation, cancellation, and explicit error/result states. Board clicks from Three.js and the CSS fallback should continue to dispatch through shared re-frame events that first update inspection state and then feed existing-territory selections in the active move flow; Cup wasteland targets are exposed through the move panel. Cup confirmations for territory and hand-card sources should build `apply-cup-move` commands for both existing-territory small-piece placement and wasteland territory creation, update `:game` only on success, and preserve the staged selection with structured gameplay errors on rejection. Move sources without pure transitions should continue to surface explicit unavailable results rather than mutating `:game` silently.

Keyboard shortcut UI state also stays browser-free in `src/main/gnostica/app_state.cljc`: `:hotkey-help-open?` tracks the help dialog, while `src/main/gnostica/app.cljs` owns the actual browser keydown listener. Keep the `?` hotkey and header control opening the command dialog, `Esc` closing it, and `I` toggling card icon mode in sync with the visible command list and smoke checks. Pressing `I` must preserve the current 3D OrbitControls view; only the explicit `Reset view` control should reset the board camera.

`src/main/gnostica/game_schema.cljc` defines Malli schemas for the pure gameplay data contract. Keep schema coverage focused on browser-free state values: card references, board cells including post-setup territory growth, player maps and six-card hand limits, Icehouse pieces including temporary wasteland coordinates, draw/discard piles, and whole-game invariants such as player counts, turn membership, piece ownership, piece space references, stash ownership, and unique card ids. Use `gnostica.game-schema/valid-game?`, `explain-game`, and `assert-valid-game` in tests, Gherkin-style feature steps, and other pure-data boundaries; do not couple these schemas to Three.js or re-frame view state.
