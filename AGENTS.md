...
`src/main/gnostica/app_state.cljc` is the browser-free app-db boundary for re-frame initialization and UI-facing state helpers. `src/main/gnostica/app/events.cljs` registers the stable `:gnostica.app/...` re-frame event and subscription ids; `:gnostica.app/initialize` delegates to `gnostica.app-state/initialize`, stores the successful game under `:game`, and keeps only UI-specific state such as `:selected-board-index` and Three.js errors at the top level. Board, piece, selected territory, and current-player subscriptions should continue to derive from `:game` instead of rebuilding board or piece state in `app.cljs` or UI view namespaces.
