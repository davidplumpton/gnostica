...
`gnostica.game-state/resolve-disc-command` delegates to `gnostica.game-state.disc` for browser-free Coins/Discs command validation. It accepts `:player-id`, a territory or hand-card source plus acting `:piece-id`, a carried `:disc-variant`, a target piece or territory, optional replacement-card source/id for territory growth, and optional orientation for current-player piece growth. It resolves normal targeting rules for upright current-space targets, cardinal adjacent-space targets, and minion self-targeting; carries variants from Coins, Magician, Strength, Star, and Sun; rejects enemy piece reorientation, enemy-occupied territory growth targets, unsupported replacement sources, unavailable variants, and invalid command shapes without mutating game state.
