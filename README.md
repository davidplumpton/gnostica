...
`gnostica.game-state/apply-rod-move` builds on `resolve-rod-command` for pure Rod piece movement. It supports moving the acting minion and pushing a target piece in the minion's facing direction, enforces positive distance up to the minion pip count, rejects upright minions, void destinations, and full destination territories, represents wasteland destinations with explicit `:space` coordinates, allows orientation changes only for moved current-player pieces, preserves enemy orientations, discards hand-source Rod cards, appends history events, and leaves Rod territory pushing as an explicit unavailable transition until that rule slice is implemented.
