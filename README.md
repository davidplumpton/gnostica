...
`gnostica.game-state/apply-cup-move` accepts an explicit command with `:player-id`, a source map for either a Cup territory or Cup hand card plus acting `:piece-id`, a target map for either an existing territory or wasteland coordinate, and the required `:orientation` or `:one-point-card-id`. It places the current player's small piece, discards hand-source Cup cards, creates one-point-card territories in legal wastelands, preserves deck and schema invariants, and reports structured rejection errors.
