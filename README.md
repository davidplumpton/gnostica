...
The main screen also displays the current player's hand, the draw deck, and the discard pile from `:game`. The hand zone shows the current player's card images and titles, the draw deck shows a facedown deck affordance with remaining-card count, and the discard pile shows an empty state until a top discarded card exists. These zones are display-only: the standalone draw/discard turn option still needs a future pure gameplay transition, while Cup and Rod hand-card move transitions already move the source card from hand to discard.

Rule helpers return explicit result maps: successful transitions use `{:ok? true :state ... :events [...]}`, while validation errors use `{:ok? false :error {:code ... :message ... :data ...}}`. `gnostica.game-state/apply-cup-move` accepts an explicit command with `:player-id`, a source map for either a Cup territory or Cup hand card plus acting `:piece-id`, a target map for either an existing territory or wasteland coordinate, and the required `:orientation` or `:one-point-card-id`. It places the current player's small piece, discards hand-source Cup cards, creates one-point-card territories in legal wastelands, preserves deck and schema invariants, and reports structured rejection errors.
