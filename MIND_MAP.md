...
[17] **Tests and Features** - Tests live under `test/gnostica`, features under `features/`, and focused regression inventories live in `docs/architecture.md`; 2026-06-18 schema audit fixed game-state Malli maps in `bd-zlun`, exposed sword-major command schemas in `bd-188f`, fixed World copied-power precedence in `bd-112n`, and rejected ambiguous Fool reveal aliases in `bd-3tfd`. Smoke targets released assets by default or `SMOKE_URL` [8][10][14][15].
